...
const ReviewSection = ({ feedbackSectionY, renderStars, calculateAverageRating }: ReviewSectionProps) => {
    ...
    const getFilteredReviews = () => {
        ...
        return MOCK_REVIEWS.filter(review => review.rating === selectedRatingFilter);
    };
            ...
            <View className="mb-4">
                ...
                <View className="flex-row flex-wrap gap-2">
                    <Pressable
                        ...
                        className={`px-4 py-2 rounded-full ${selectedRatingFilter === 'all' ? 'bg-red-500' : 'bg-gray-200'
                            }`}
                    >
                        <Text className={`font-semibold ${selectedRatingFilter === 'all' ? 'text-white' : 'text-gray-700'
                            }`}>
                            All
                        </Text>
                    </Pressable>
                    ...
                    {[5, 4, 3, 2, 1].map((rating) => (
                        <Pressable
                            ...
                            className={`px-4 py-2 rounded-full flex-row items-center ${selectedRatingFilter === rating ? 'bg-red-500' : 'bg-gray-200'
                                }`}
                        >
                            <Ionicons
                                name="star"
                            ...
                            />
                            <Text className={`font-semibold ml-1 ${selectedRatingFilter === rating ? 'text-white' : 'text-gray-700'
                                }`}>
                                {rating}
                            </Text>
                        </Pressable>
            ...
            <View className="mb-6">
                ...
                {getFilteredReviews().length === 0 ? (
                    ...
                    getFilteredReviews().map((review) => (
                        <View key={review.id} className="mb-6 pb-4 border-b border-gray-100">
